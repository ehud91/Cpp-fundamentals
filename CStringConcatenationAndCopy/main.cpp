...
int main() {
    ...
    std::strcat(dest, " Goodbye World!");   // Some compilers (MSVC) think these functions are unsafe!
    std::cout << "dest : " << dest << std::endl;
    ...
    char * source1 =
            ...
            // NOTE TO SELF :
            // Describe is going to be appended to dest, and the process
            // will start by overriding the null character in dest

    std::cout << "std::strlen(dest1) : " << std::strlen(dest1) << std::endl;
    std::cout << "std::strlen(souce1) : " << std::strlen(source1) << std::endl;
    ...
    // This is patter you'll see a lot in C++ code out there
